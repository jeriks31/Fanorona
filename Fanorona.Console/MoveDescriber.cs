using System.Numerics;
using Fanorona.Core;

namespace Fanorona.ConsoleApp;

public static class MoveDescriber
{
    /// <summary>One-line form used in move lists and selection prompts.</summary>
    public static string Summary(Turn turn) => turn.IsCapture
        ? $"{Notation.Format(turn)}  (captures {CellList(turn.CapturedMask)})"
        : $"{Notation.Format(turn)}  (no capture)";

    /// <summary>Step-by-step English instructions for playing the turn in AC.</summary>
    public static IEnumerable<string> DescribeSteps(Turn turn)
    {
        if (!turn.IsCapture)
        {
            yield return $"Move {turn.Origin} to {turn.Destination} — no capture (paika).";
            yield break;
        }
        for (var i = 0; i < turn.Segments.Length; i++)
        {
            var segment = turn.Segments[i];
            var verb = i == 0 ? "Move" : "then";
            var kind = segment.Kind == CaptureKind.Approach ? "approach" : "withdrawal";
            yield return $"{verb} {segment.From} to {segment.To} — {kind}, removing {CellList(segment.Captured)}.";
        }
    }

    private static string CellList(ulong mask)
    {
        var cells = new List<string>();
        for (var bits = mask; bits != 0; bits &= bits - 1)
            cells.Add(new Cell(BitOperations.TrailingZeroCount(bits)).ToString());
        return string.Join(", ", cells);
    }
}
